...
const RankingWidget: React.FC<RankingWidgetProps> = ({ onSetupProfile }) => {
  ...
  const loadRankings = useCallback(
    async (showLoading = true) => {
      console.log("ðŸ”„ loadRankings called, showLoading:", showLoading);
      try {
        if (showLoading) {
        ...
        if (
          ...
          // NO_PROFILE error
          console.log("ðŸš« No profile found, setting hasProfile to false");
          setHasProfile(false);
          setError("NO_PROFILE");
          setIsCalculating(false);
        } else if (
          ...
          console.log("â³ Rankings not ready, starting/continuing polling");
          if (pollCount < MAX_POLLS) {
            setIsCalculating(true);
            setHasProfile(true);
            setError(null);
          } else {
            console.log("â±ï¸ Max polls reached, stopping");
            setError("TIMEOUT");
            setIsCalculating(false);
          }
        } else if (
          ...
          console.log("âŒ Server error:", err);
          ...
          // Invalid score error
          console.log("âŒ Invalid score error:", err);
          setError("INVALID_SCORE");
          setIsCalculating(false);
        } else {
          // Generic error
          console.log("âŒ Generic rankings error:", err);
          setError("GENERIC_ERROR");
          setIsCalculating(false);
        }
      } finally {
  ...
  useEffect(() => {
    ...
    console.log(
      `â³ Polling for rankings (attempt ${pollCount + 1}/${MAX_POLLS})`
    );
  ...
  console.log("ðŸŽ¨ RankingWidget render state:", {
    isLoading,
    hasProfile,
    error,
    rankings: !!rankings,
