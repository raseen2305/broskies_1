...
import { useNavigate } from 'react-router-dom';
import { motion } from 'framer-motion';
import { TrendingUp, GitBranch, Code, Shield, Award, Activity, Calendar } from 'lucide-react';
import { useAuth } from '../../contexts/AuthContext';
import { scanAPI } from '../../services/api';
import RankingWidget from '../RankingWidget';
import ExternalRankingWidget from '../ExternalRankingWidget';
import ProfileSetupModal from '../ProfileSetupModal';
import { useScoreSync, dispatchScanCompleteEvent } from '../../hooks/useScoreSync';
import AnalyzeButton from '../AnalyzeButton';
import AnalysisProgress from '../AnalysisProgress';
import AnimatedQualityWheel from './AnimatedQualityWheel';
...
=======
      console.log('🔍 Checking if user has profile...');
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/profile/status`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
        }
      });
...
=======
        ...
        const transformedStats: UserStats = {
          ...
          languages: languagesData,
          githubProfile: scanResults.userInfo ? {
            username: scanResults.userInfo.login || scanResults.userInfo.username || scanResults.targetUsername,
            name: scanResults.userInfo.name,
            bio: scanResults.userInfo.bio,
            location: scanResults.userInfo.location,
            company: scanResults.userInfo.company,
        ...
        if (user?.id && transformedStats.overallScore > 0) {
          ...
          // If not, show profile setup modal
          // Check if this is an internal scan (not scanning someone else)
          const isInternalScan = scanResults.scanType === 'self' || 
                                 scanResults.scanType === 'myself' || 
                                 (scanResults.scanType !== 'external_user' && 
                                  scanResults.scanType !== 'external' && 
                                  !scanResults.isExternalScan &&
                                  (!scanResults.targetUsername || scanResults.targetUsername === user.githubUsername));
      ...
      // and we're not in a scanning/external user context
      if (!user?.id) {
        setError('Please log in to view your dashboard or scan a GitHub profile.');
        setIsLoading(false);
        return;
      }
      ...
      // Don't automatically load authenticated user data
      // Let the user explicitly scan their own profile if needed
      setError('Welcome! Please scan a GitHub profile to view detailed analytics and insights.');
>>>>>>> d5e7869ebe813aaf39e98e4cc56498e93f572085
      setIsLoading(false);
    };
...
=======
  const stats = userStats ? [
    // Only include Overall Score if repos have been evaluated
    ...(scanResults && (scanResults.evaluatedCount > 0 || scanResults.analyzed) ? [{
      label: 'Overall Score', 
      value: userStats.overallScore.toFixed(1), 
      icon: Award, 
      color: 'text-primary-500',
    ...
    { 
      ...
      color: 'text-secondary-500',
      bgColor: 'bg-secondary-50',
      change: userStats.githubProfile ? `${userStats.githubProfile.public_repos} total` : 'GitHub'
    },
    { 
      label: 'Languages Used', 
      value: userStats.languages.length.toString(), 
      ...
      color: 'text-accent-500',
      bgColor: 'bg-accent-50',
      change: userStats.languages.length > 0 ? `${userStats.languages[0]?.language} primary` : 'Various'
    },
    { 
      label: 'GitHub Followers', 
      value: userStats.githubProfile?.followers?.toString() || '0', 
      ...
      color: 'text-success-500',
      bgColor: 'bg-success-50',
      change: userStats.githubProfile?.following ? `${userStats.githubProfile.following} following` : 'GitHub'
    },
  ] : [];
>>>>>>> d5e7869ebe813aaf39e98e4cc56498e93f572085
    ...
    return (
      <div className="space-y-6">
        <div>
          ...
          <p className="text-gray-600">
            {isLoadingError ? 'Your account is being prepared' : 'Scan a GitHub profile to get started'}
          </p>
        </div>
        <div className="card p-8 text-center">
          <div className={`${isLoadingError ? 'text-blue-500' : 'text-orange-500'} mb-4`}>
            {isLoadingError ? (
...
=======
                <p>If this persists, try refreshing the page or scanning a different profile.</p>
>>>>>>> d5e7869ebe813aaf39e98e4cc56498e93f572085
              </div>
            )}
          </div>
    ...
    <div className="space-y-6">
      <div>
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 mb-2">
              {scanResults?.targetUsername ? `${scanResults.targetUsername}'s Profile` : 'Dashboard Overview'}
            </h1>
            <p className="text-gray-600">
              {scanResults?.targetUsername 
              ...
              {scanResults.targetUsername && (
                <div className="flex items-center space-x-2 bg-blue-50 text-blue-700 px-4 py-2 rounded-lg border border-blue-200 mr-2">
                  <span className="text-sm font-medium">
                    {scanResults.scanType === 'self' ? 'Your Profile' : 'External Profile Scan'}
                  </span>
                </div>
              )}
      ...
      {scanResults && (!scanResults.overallScore || (scanResults.evaluatedCount === 0 && !scanResults.analyzed)) && scanResults.targetUsername && (
        <motion.div
      ...
      {scanResults && scanResults.overallScore > 0 && scanResults.repositories && scanResults.repositories.some((r: any) => r.analysis?.acid_scores?.overall) && (
        <motion.div
          initial={{ opacity: 0, y: -10 }}
          animate={{ opacity: 1, y: 0 }}
          className="mb-4"
          ...
          <div className="card p-4 sm:p-5 bg-gradient-to-r from-blue-50 to-indigo-50 border-2 border-blue-200">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs sm:text-sm text-gray-700 mb-2">
                      To view each repository's individual scores and detailed metrics, move to the Repositories tab.
                    </p>
                    <p className="text-xs text-blue-700 flex flex-wrap gap-x-2">
                      <span>{scanResults.repositories.filter((r: any) => r.category === 'flagship').length} Flagship</span>
                      <span>•</span>
                      <span>{scanResults.repositories.filter((r: any) => r.category === 'significant').length} Significant</span>
                      <span>•</span>
                      <span>{scanResults.repositories.filter((r: any) => r.analysis?.acid_scores?.overall).length} Evaluated</span>
                    </p>
                  </div>
                </div>
              </div>
              <button
                onClick={() => navigate('/developer/dashboard/repositories')}
              ...
              >
                ...
                <span className="text-sm sm:text-base">View Repositories</span>
              </button>
            </div>
>>>>>>> d5e7869ebe813aaf39e98e4cc56498e93f572085
          </div>
...
=======
      {/* Analysis Section - Show quality metrics ONLY for evaluated scans */}
      {scanResults && scanResults.overallScore > 0 && (scanResults.evaluatedCount > 0 || scanResults.analyzed) && (
        <>
          {/* Show Analyze Button if not analyzed (external scans only) */}
          {scanResults?.scanType === 'other' && !scanResults?.analyzed && !isAnalyzing && (
            <AnalyzeButton
              username={scanResults.targetUsername}
              analyzed={false}
              onAnalysisComplete={() => {
          ...
          {/* Show Analysis Progress if in progress (external scans only) */}
          {scanResults?.scanType === 'other' && isAnalyzing && analysisStatus && (
            <AnalysisProgress
              status={analysisStatus.status}
              progress={analysisStatus.progress}
              message={analysisStatus.message}
          ...
          {scanResults?.overallScore && 
           ...
           (
            // Show for internal scans (self/myself) OR external scans that have been analyzed
            (scanResults.scanType === 'myself' || scanResults.scanType === 'self' || scanResults.analyzed)
          ) && (
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              <div className="text-center mb-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  {scanResults.scanType === 'myself' || scanResults.scanType === 'self' 
                    ? 'Your Code Quality Score' 
                    : 'Code Quality Analysis'}
                </h2>
                <p className="text-gray-600">
                  Based on {scanResults.evaluatedCount || scanResults.repositoryCount || 0} evaluated repositories
                  {scanResults.analyzedAt && (
                    <span className="ml-2 text-sm">
                      • Analyzed {new Date(scanResults.analyzedAt).toLocaleDateString()}
                    </span>
                  )}
                  {!scanResults.analyzedAt && (
                    <span className="ml-2 text-sm">
              ...
              {/* Animated Quality Wheel */}
              <div className="flex items-center justify-center mb-8" style={{ minHeight: '500px' }}>
                <AnimatedQualityWheel
                  qualityMetrics={{
                    readability: scanResults.qualityMetrics?.readability || 
                                scanResults.repositories?.[0]?.evaluation?.quality_metrics?.readability || 75,
                    maintainability: scanResults.qualityMetrics?.maintainability || 
                                    scanResults.repositories?.[0]?.evaluation?.quality_metrics?.maintainability || 70,
                    security: scanResults.qualityMetrics?.security || 
                             scanResults.repositories?.[0]?.evaluation?.quality_metrics?.security || 85,
                    test_coverage: scanResults.qualityMetrics?.test_coverage || 
                                  scanResults.repositories?.[0]?.evaluation?.quality_metrics?.test_coverage || 60,
                    documentation: scanResults.qualityMetrics?.documentation || 
                                  scanResults.repositories?.[0]?.evaluation?.quality_metrics?.documentation || 65,
                    performance: 75,
                    complexity: 70,
                    best_practices: 80
                  }}
              ...
              {/* Repository Distribution */}
              {(scanResults.flagshipCount > 0 || scanResults.significantCount > 0) && (
                <div className="grid grid-cols-3 gap-4 mb-6">
                  <div className="text-center p-4 bg-yellow-50 rounded-lg border border-yellow-200">
                    <div className="text-3xl font-bold text-yellow-700">
                      {scanResults.flagshipCount || 0}
                    </div>
                    <div className="text-sm text-yellow-600 font-medium">🏆 Flagship</div>
                  </div>
                  <div className="text-center p-4 bg-blue-50 rounded-lg border border-blue-200">
                    <div className="text-3xl font-bold text-blue-700">
                      {scanResults.significantCount || 0}
                    </div>
                    <div className="text-sm text-blue-600 font-medium">⭐ Significant</div>
                  </div>
                  <div className="text-center p-4 bg-gray-50 rounded-lg border border-gray-200">
                    <div className="text-3xl font-bold text-gray-700">
                      {scanResults.supportingCount || 0}
                    </div>
                    <div className="text-sm text-gray-600 font-medium">📋 Supporting</div>
                  </div>
                </div>
              )}
              ...
              {scanResults.scoreBreakdown && (
                <div className="space-y-3">
                  ...
                  {scanResults.scoreBreakdown.components?.map((component: any) => (
                    <div key={component.category} className="flex items-center justify-between">
                      <div className="flex items-center space-x-2">
                        <span className="text-sm font-medium text-gray-700">
                          {component.category === 'flagship' ? '🏆' : '⭐'} {component.category.charAt(0).toUpperCase() + component.category.slice(1)}
                        </span>
                        <span className="text-xs text-gray-500">
                          ({component.count} repos, {component.percentage} weight)
                        </span>
                      </div>
                      <div className="flex items-center space-x-2">
                        <div className="text-sm font-semibold text-gray-900">
      ...
      {userStats?.githubProfile && (
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-gray-900">
              {scanResults?.scanType === 'other' ? 'User Profile' : 'Your GitHub Profile'}
            </h3>
            <a
              href={`https://github.com/${userStats.githubProfile.username}`}
              target="_blank"
          ...
          <div className="flex items-start space-x-6">
            ...
            <div className="flex-1">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div>
                  <h4 className="text-xl font-semibold text-gray-900">
                    {userStats.githubProfile.name || userStats.githubProfile.username}
                  </h4>
                  <p className="text-sm text-gray-600 mb-2">@{userStats.githubProfile.username}</p>
                  {userStats.githubProfile.bio && (
                    <p className="text-sm text-gray-700 mb-4 leading-relaxed">{userStats.githubProfile.bio}</p>
                  )}
                  <div className="space-y-2">
                    {userStats.githubProfile.location && (
                      <div className="flex items-center text-sm text-gray-600">
                    ...
                    {userStats.githubProfile.blog && (
                      <div className="flex items-center text-sm text-gray-600">
                        <span className="mr-2">🔗</span>
                        <a 
                          href={userStats.githubProfile.blog.startsWith('http') ? userStats.githubProfile.blog : `https://${userStats.githubProfile.blog}`}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-blue-600 hover:text-blue-800"
                        >
                    ...
                    {userStats.githubProfile.created_at && (
                      <div className="flex items-center text-sm text-gray-600">
                        <span className="mr-2">📅</span>
                        <span>Joined {new Date(userStats.githubProfile.created_at).toLocaleDateString('en-US', { 
                          year: 'numeric', 
                          month: 'long' 
                        })}</span>
                      </div>
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="text-center p-4 bg-gray-50 rounded-lg">
                    <div className="text-2xl font-bold text-gray-900">{userStats.githubProfile.public_repos}</div>
                    <div className="text-xs text-gray-600 mt-1">Public Repos</div>
                  </div>
                  <div className="text-center p-4 bg-gray-50 rounded-lg">
                    <div className="text-2xl font-bold text-gray-900">{userStats.githubProfile.followers}</div>
                    <div className="text-xs text-gray-600 mt-1">Followers</div>
                  </div>
                  <div className="text-center p-4 bg-gray-50 rounded-lg">
                    <div className="text-2xl font-bold text-gray-900">{userStats.githubProfile.following}</div>
      ...
      {(() => {
        // Determine if this is an internal or external scan
        const isInternalScan = !scanResults || 
          scanResults.scanType === 'self' || 
          scanResults.scanType === 'myself' || 
          (scanResults.scanType !== 'external_user' && 
           scanResults.scanType !== 'external' && 
        ...
        const isExternalScan = scanResults && !isInternalScan && scanResults.targetUsername;
        ...
        if (isInternalScan) {
          ...
          return (
            <motion.div
              ...
              transition={{ delay: 0.4 }}
            >
              <RankingWidget onSetupProfile={() => setIsProfileModalOpen(true)} />
            </motion.div>
          );
        } else if (isExternalScan) {
          // External scan - show scanned user's rankings by GitHub username
      ...
      {scanResults?.contributionStats && (
        ...
        >
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <div className="text-center p-4 bg-green-50 rounded-lg">
              <div className="text-2xl font-bold text-green-600">
                {scanResults.contributionStats.contribution_streaks?.current_streak || 0}
              </div>
              <div className="text-xs text-gray-600 mt-1">Current Streak</div>
            </div>
            <div className="text-center p-4 bg-blue-50 rounded-lg">
              <div className="text-2xl font-bold text-blue-600">
                {scanResults.contributionStats.contribution_streaks?.longest_streak || 0}
              </div>
              <div className="text-xs text-gray-600 mt-1">Longest Streak</div>
            </div>
            <div className="text-center p-4 bg-purple-50 rounded-lg">
              <div className="text-2xl font-bold text-purple-600">
                {scanResults.contributionStats.contribution_patterns?.total_active_days || 0}
              </div>
              <div className="text-xs text-gray-600 mt-1">Active Days</div>
            </div>
            <div className="text-center p-4 bg-orange-50 rounded-lg">
              <div className="text-2xl font-bold text-orange-600">
                {scanResults.contributionStats.contribution_patterns?.most_active_day?.day || 'N/A'}
              </div>
              <div className="text-xs text-gray-600 mt-1">Most Active Day</div>
            </div>
          </div>
          ...
          {scanResults.contributionStats.calendar_data && (
            <div className="mb-4">
              ...
              <div className="flex flex-wrap gap-1">
                {scanResults.contributionStats.calendar_data.slice(-84).map((day: any, index: number) => (
                  <div
                    key={day.date}
                    className="w-3 h-3 rounded-sm"
                    style={{ backgroundColor: day.color }}
...
=======
          ...
          {scanResults.contributionStats.commit_repositories && (
            <div>
              <h4 className="text-sm font-medium text-gray-700 mb-3">Top Contributing Repositories</h4>
              <div className="space-y-2">
                {scanResults.contributionStats.commit_repositories.slice(0, 5).map((repo: any) => (
                  <div key={repo.name} className="flex items-center justify-between p-2 bg-gray-50 rounded">
                    <div className="flex items-center space-x-3">
                      <div 
                        className="w-3 h-3 rounded-full"
                        style={{ backgroundColor: repo.language_color }}
                    ...
                    <span className="text-sm text-gray-600">{repo.contributions} commits</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      {userStats && userStats.languages.length > 0 && (
        ...
        >
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-xl font-semibold text-gray-900">Language Distribution</h3>
            <button 
              onClick={() => window.location.href = '/developer/dashboard/languages'}
              className="text-primary-500 hover:text-primary-600 text-sm font-medium"
            >
              View All →
            </button>
      ...
      >
        <div className="card p-6">
          <h3 className="text-xl font-semibold text-gray-900 mb-4">
            {scanResults?.targetUsername ? `${scanResults.targetUsername}'s Summary` : 'Profile Summary'}
          </h3>
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <span className="text-gray-600">GitHub Username</span>
            ...
            <div className="flex items-center justify-between">
              <span className="text-gray-600">Repositories Analyzed</span>
              <span className="font-medium text-gray-900">{userStats?.repositoryCount || 0}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-gray-600">Scan Date</span>
              <span className="font-medium text-gray-900">
            ...
            {scanResults?.evaluation_method && (
              <div className="flex items-center justify-between">
                <span className="text-gray-600">Evaluation Method</span>
                <span className="font-medium text-gray-900">
                  {scanResults.evaluation_method.includes('ml') ? 'AI + ACID' : 'ACID Scoring'}
                </span>
              </div>
            )}
          </div>
        ...
        {scanResults?.repositoryOverview && (
          <div className="card p-6">
            <h3 className="text-xl font-semibold text-gray-900 mb-4">Repository Overview</h3>
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="text-center p-3 bg-blue-50 rounded-lg">
                  <div className="text-lg font-bold text-blue-600">
                    {scanResults.repositoryOverview.original_repositories || 0}
                  </div>
                  <div className="text-xs text-gray-600">Original Repos</div>
                </div>
                <div className="text-center p-3 bg-green-50 rounded-lg">
              ...
              {scanResults.repositoryOverview.most_starred && (
                <div className="p-3 bg-yellow-50 rounded-lg">
                  <div className="text-sm font-medium text-gray-900">Most Starred Repository</div>
                  <div className="text-sm text-gray-600">
                    <strong>{scanResults.repositoryOverview.most_starred.name}</strong>
                    {scanResults.repositoryOverview.most_starred.language && (
                      <span className="ml-2 text-xs bg-gray-200 px-2 py-1 rounded">
                        {scanResults.repositoryOverview.most_starred.language}
                      </span>
              ...
              {scanResults.repositoryOverview.recently_updated && scanResults.repositoryOverview.recently_updated.length > 0 && (
                <div>
                  <div className="text-sm font-medium text-gray-900 mb-2">Recently Updated</div>
                  <div className="space-y-1">
                    {scanResults.repositoryOverview.recently_updated.slice(0, 3).map((repo: any) => (
            ...
            {scanResults?.targetUsername ? (
              <>
                <div className="flex items-start space-x-3">
                  <div className="w-2 h-2 bg-primary-500 rounded-full mt-2"></div>
                  <div>
                    <p className="font-medium text-gray-900">Profile Analysis Complete</p>
                    <p className="text-sm text-gray-600">
                      Analyzed {userStats?.repositoryCount || 0} repositories from {scanResults.targetUsername}
                    </p>
                  </div>
                </div>
                <div className="flex items-start space-x-3">
                  <div className="w-2 h-2 bg-secondary-500 rounded-full mt-2"></div>
                  <div>
                    <p className="font-medium text-gray-900">Technology Stack Identified</p>
                    <p className="text-sm text-gray-600">
                      Primary focus on {userStats?.languages[0]?.language || 'various technologies'}
                    </p>
                  </div>
                </div>
                <div className="flex items-start space-x-3">
                  <div className="w-2 h-2 bg-accent-500 rounded-full mt-2"></div>
                  <div>
                    <p className="font-medium text-gray-900">Scan Another User</p>
                    <p className="text-sm text-gray-600">
                      <button 
                        onClick={() => window.location.href = '/developer/auth'}
                        className="text-blue-600 hover:text-blue-800"
              ...
              <>
                <div className="flex items-start space-x-3">
                  <div className="w-2 h-2 bg-secondary-500 rounded-full mt-2"></div>
                  <div>
                    <p className="font-medium text-gray-900">Explore your tech stack</p>
                    <p className="text-sm text-gray-600">See what technologies you're using across projects</p>
                  </div>
                </div>
                <div className="flex items-start space-x-3">
                  <div className="w-2 h-2 bg-accent-500 rounded-full mt-2"></div>
                  <div>
                    <p className="font-medium text-gray-900">Check your roadmap</p>
                ...
                <div className="flex items-start space-x-3">
                  <div className="w-2 h-2 bg-primary-500 rounded-full mt-2"></div>
                  <div>
                    <p className="font-medium text-gray-900">View detailed repositories</p>
                    <p className="text-sm text-gray-600">Explore individual repository analysis and metrics</p>
                  </div>
                </div>
              </>
            )}
