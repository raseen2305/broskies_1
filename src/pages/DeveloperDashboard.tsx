  ...
  const loadDefaultProfile = async () => {
    try {
      ...
      if (authToken && authUser?.githubUsername) {
        ...
        console.log(
          "🔍 Authenticated user, fetching existing dashboard data..."
        );

        try {
          const { profileAPI } = await import("../services/profileAPI");
          console.log(
            "🔍 Attempting to fetch dashboard data for authenticated user..."
          );
          const dashboardData = await profileAPI.getDashboardData();
          ...
          if (dashboardData.has_data) {
            ...
            // Set the scan results from existing data
            setScanResults(dashboardData);
            setError(null);
            setIsLoadingProfile(false);
            return; // Exit early since we have data
          } else {
            console.log("📝 No existing scan data, showing scan prompt");
            console.log("📝 Dashboard data message:", dashboardData.message);
            setError(
